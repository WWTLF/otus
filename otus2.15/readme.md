...
|13|Если проверка баланса прошла успешно, сервис профиля генерирует событие ORDER _REJECTED |Событие ORDER _REJECTED, контекст   события error, [спецификация](https://gitlab.com/portfolio_counselor/profile-src/-/blob/master/profile.yaml)|FR 3.3|
...
2. Сервис ведения портфеля проверяет остаток свободных для инвестирования средств по таблице potrfolios полю free_amount
    1. Если средств достаточно:
        ...
        2. Сервис отправляет событие в оркестратор: ORDER _APPROVED
    2. Если средств недостаточно:
        1. Сервис отправляет событие в оркестратор: ORDER _REJECTED
